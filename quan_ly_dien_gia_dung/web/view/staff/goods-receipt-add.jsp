...
<html>
    ...
    <body>
        ...
        <script>
            ...
            let searchTimeout;
            const STORAGE_KEY = 'goods_receipt_draft';
            
            // Set min date to today (prevent selecting past dates)
            const today = new Date().toISOString().split('T')[0];
            document.getElementById('receiptDate').setAttribute('min', today);
            
            // Auto-set today's date
            document.querySelector('input[name="receiptDate"]').valueAsDate = new Date();
            
            // Load from localStorage first (for page reload)
            ...
            // Restore products if validation failed (from server)
            <c:if test="${not empty productsJson}">
                try {
                    // Clear table first
                    $('#productTableBody').empty();
                    
                    const restoredProducts = JSON.parse('${productsJson}');
                    products = [];
                    productIdCounter = 1;
            ...
            <c:if test="${empty productsJson}">
                // Load from localStorage only if no server data
                loadFromStorage();
            </c:if>
            ...
            function loadProducts(searchValue) {
                $.ajax({
                    url: '${pageContext.request.contextPath}/goods-receipt-add',
                    type: 'POST',
                    data: {
                        ...
                        <td>
                            ...
                            <div class="mt-1">
                                <button type="button" class="btn btn-sm btn-outline-primary serial-btn" data-id="\${product.id}">
                                    <i class="fas fa-barcode me-1"></i>Nhập serial
                                </button>
                            </div>
                        </td>
                        <td>\${product.name}</td>
            ...
            function renderSerialTags() {
                ...
                currentSerials.forEach((serial, index) => {
                    tagsHtml += `<span class="tag">\${serial} <span class="remove-tag" onclick="removeSerialTag(\${index})">×</span></span>`;
                });
                
                const html = `
                    <div class="tag-input-container" onclick="document.getElementById('serialTagInput').focus()">
                        \${tagsHtml}
                        <input type="text" class="tag-input" id="serialTagInput" 
